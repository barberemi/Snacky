...
/// Implémentation mock du [ArticleService].
/// Simule les réponses de l'API Rust avec des données en dur + génération dynamique.
///
/// Remplacer par `ApiArticleService` quand l'API sera disponible.
class MockArticleService implements ArticleService {
  ...
  Future<List<Article>> fetchArticlesByTag({
    ...
    final all = _getFixtureArticles();
    final known = all
        .where((a) => a.tags.any((t) => t.toLowerCase() == tag.toLowerCase()))
        .toList();
    if (known.isEmpty) {
      return _simulateNetwork(_generateArticlesForTag(tag));
    }
  ...
  Future<List<Article>> fetchAllArticles({required String userId}) async {
    return _simulateNetwork(_getFixtureArticles());
  }

  // ─── Fixtures ─────────────────────────────────────────────────────────────

  List<Article> _getFixtureArticles() {
    return [
      Article(
        id: '1',
  ...
  // ─── Génération dynamique pour les tags inconnus ──────────────────────────

  List<Article> _generateArticlesForTag(String tag) {
    return [
      Article(
        id: '${tag}_mock_1',
